...
function remapAnswers(a, productSelection) {
  ...
  if (selectedType === 'pill' && !transdermalSideEffects && !nicotineOrClot) {
    ...
    hrtFormulation       = 'Estradiol patch';
    hrtSideEffects       = 'Yes';
    hrtSideEffectsDetail = 'Skin irritation at application site';
    transdermalSE        = 'Yes';
    transdermalReaction  = 'Skin irritation at application site';
  }
...
// We NEVER modify the products= param.
function appendCheckoutParams(url, clickId) {
  ...
  // Append click ID for attribution tracking if present
  if (clickId) {
    result += '&cc_custom_cid=' + encodeURIComponent(clickId);
  }
  return result;
}
