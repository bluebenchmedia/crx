...
(function() {
  ...
  function startFinalSubmission() {
    // V1: Call /api/v1/complete during the loading animation.
    // No treatment selection — Dosable decides the product based on honest answers.
    ...
    // Animate the loading bar while the API call runs
    ...
    fetch(PROXY_BASE + '/api/v1/complete', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId:    sessionId,
    ...
    .then(function(data) {
      ...
      if (!data.ok) {
        ...
        console.error('v1 complete failed:', data);
        return;
      }
      // Store result for treatment page
      sessionStorage.setItem('crx_v1_result', JSON.stringify({
    ...
    .catch(function(err) {
      ...
      if (loadingStatus) loadingStatus.textContent = 'Connection error. Please check your internet and try again.';
